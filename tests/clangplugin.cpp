...
void
report(const CompilerInstance & compiler,
...
#endif
    virtual bool
    ParseArgs(const CompilerInstance & compiler, const vector<string> & args);
...
public:
    ...
    bool
    VisitReturnStmt(const ReturnStmt * returnstmt);
    virtual void
...
Action::CreateASTConsumer(CompilerInstance & compiler, StringRef)
{
...
bool
Action::ParseArgs(const CompilerInstance & /*compiler*/,
...
Consumer::Consumer(CompilerInstance & compiler, const vector<string> & args)
    : compiler(compiler)
{
    // Check that the file passed as argument really exists (was included in
    // ICECC_EXTRAFILES).
    if (args.size() != 1) {
        report(compiler,
    ...
    if (!is.good())
        ...
        if (strcmp(buf, "testfile") != 0)
            ...
            report(compiler,
                   ...
                   "Extra file check successful");
    }
}

void
Consumer::HandleTranslationUnit(ASTContext & context)
{
...
bool
Consumer::VisitReturnStmt(const ReturnStmt * returnstmt)
{
    ...
    // Check if the expression is a bool literal (Clang uses dyn_cast<> instead
    // of dynamic_cast<>).
    if (const CXXBoolLiteralExpr * boolliteral =
        ...
        if (boolliteral->getValue() == false)
            report(compiler,
                   DiagnosticsEngine::Warning,
                   "Icecream plugin found return false",
       ...
       SourceLocation           loc)
{
    ...
    if (loc.isValid())
        engine.Report(loc,
                      engine.getDiagnosticIDs()->getCustomDiagID(
...
static FrontendPluginRegistry::Add<IcecreamTest::Action>
    X("icecreamtest", "Icecream test plugin");
