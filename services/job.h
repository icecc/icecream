...
public:
    ...
    CompileJob()
        : m_id(0), m_dwarf_fission(false), m_block_rewrite_includes(false)
    {
        setTargetPlatform();
    }

    void
    setCompilerName(const std::string & name)
    {
    ...
    // Not used remotely.
    void
    setCompilerPathname(const std::string & pathname)
    {
    ...
    std::string
    compilerPathname() const
    {
        return m_compiler_pathname;
    }

    void
    setEnvironmentVersion(const std::string & ver)
    {
        m_environment_version = ver;
    }

    std::string
    environmentVersion() const
    {
    ...
    void
    setFlags(const ArgumentsList & flags)
    {
        m_flags = flags;
    }
    std::list<std::string>
    ...
    void
    setInputFile(const std::string & file)
    {
        m_input_file = file;
    }
    ...
    // Since protocol 41 this is just a shortcut saying that allFlags() contains
    // "-gsplit-dwarf".
    void
    setDwarfFissionEnabled(bool flag)
    {
        m_dwarf_fission = flag;
    }

    bool
    dwarfFissionEnabled() const
    {
        return m_dwarf_fission;
    }

    void
    setWorkingDirectory(const std::string & dir)
    {
    ...
    std::string
    targetPlatform() const
    {
        return m_target_platform;
    }

    void
    setTargetPlatform(const std::string & _target)
    {
    ...
    // Not used remotely.
    void
    setBlockRewriteIncludes(bool flag)
    {
...
inline void
appendList(std::list<std::string> & list, const std::list<std::string> & toadd)
{
...
inline std::ostream &
operator<<(std::ostream & output, const CompileJob::Language & l)
{
    switch (l) {
        case CompileJob::Lang_CXX: output << "C++"; break;
        case CompileJob::Lang_C: output << "C"; break;
        case CompileJob::Lang_Custom: output << "<custom>"; break;
        case CompileJob::Lang_OBJC: output << "ObjC"; break;
        case CompileJob::Lang_OBJCXX: output << "ObjC++"; break;
    }
    return output;
}

inline std::string
concat_args(const std::list<std::string> & args)
{
