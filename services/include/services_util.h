...
// These two detect if the given binary is a C/C++ compiler based on its
// name(gcc->C,clang++->C++)
bool
is_c_compiler(const std::string & compiler);
bool
is_cpp_compiler(const std::string & compiler);
// These two return the given binary for the C or C++ compiler based on a
// compiler. E.g. get_c_compiler("clang++-8") -> "clang-8".
std::string
...
// If check_errors is set, then errors such as POLLHUP are also considered as
// matching.
bool
