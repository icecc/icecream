...
static void handle_scheduler_announce(char* buf, const char* netname, int schedbuflen, bool persistent_clients)
{
    ...
    if (!persistent_clients){
        ...
        {
            buf[schedbuflen] = '\0';
            recv_netname = strdup(buf + 5 + sizeof(uint64_t));
    ...
    while (!exit_main_loop) {
        ...
        if (max_fd && FD_ISSET(broad_fd, &read_set)) {
            ...
            if (buflen == 1) {
                ...
                handle_scheduler_announce(buf, netname, schedbuflen, persistent_clients);
            }
        }
