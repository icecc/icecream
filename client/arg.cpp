...
static bool
analyze_assembler_arg(string & arg, list<string> * extrafiles)
{
             ...
             list<string> *       extrafiles)
{
    ...
    for (int i = had_cc ? 2 : 1; argv[i]; i++) {
        ...
        if (icerun) {
            ...
            if (!strcmp(a, "-E")) {
                ...
                args.append(a, Arg_Local);
                log_warning()
                    << "argument " << a << ", building locally" << endl;
                ...
                */
                ...
                if (const char * opt = argv[i + 1]) {
                    ...
                    if (str_equal(opt, "c++") || str_equal(opt, "c") ||
                        ...
                        job.setLanguage(
                            lang); // will cause -x used remotely twice, but
                ...
                if (ofile == "-") {
                     ...
                     * pretty rare case. */
                    log_warning()
                        << "output to stdout?  running locally" << endl;
                    always_local = true;
                }
                ...
                if (seen_arch) {
                    log_warning()
                        << "multiple -arch options, building locally" << endl;
                    always_local = true;
                }
                seen_arch = false;
                args.append(a, Arg_Rest);
    ...
    if (!always_local) {
        ...
        for (ArgumentsList::iterator it = args.begin(); it != args.end();) {
            if (it->first == "-") {
                always_local = true;
                log_warning()
                    << "stdin/stdout argument, building locally" << endl;
                break;
            }
            ...
            if (it->first == "-Xclang" || it->first == "-x" ||
                ...
                if (should_always_build_locally(ifile)) {
                    log_info()
                        << "configure tests are run locally: " << ifile << endl;
                    always_local = true;
                }
            } else {
    ...
    // redirecting compiler's output will turn off its automatic coloring, so
    // force it when it would be used, unless explicitly set
    ...
    // -fdirectives-only outright refuses to work if -Wunused-macros is given,
    // and Clang's -frewrite-includes is buggy if -Wunused-macros is used
    // (https://bugs.llvm.org/show_bug.cgi?id=15614). It's a question if this
    // could possibly even work, given that macros may be used as filenames for
    // #include directives.
    if (wunused_macros) {
        job.setBlockRewriteIncludes(true);
    }

    // -pedantic doesn't work with remote preprocessing, if extensions to a
    ...
    if (!always_local && compiler_only_rewrite_includes(job) &&
    ...
    if (!always_local && !seen_target && compiler_is_clang(job)) {
        // With gcc each binary can compile only for one target, so
        // cross-compiling is just a matter of using the proper cross-compiler
        // remotely and it will automatically compile for the given platform.
        // However one clang binary can compile for many platforms and so when
    ...
    if (!always_local &&
        (seen_march_native || seen_mcpu_native || seen_mtune_native)) {
        // These depend on the host where the compilation is done, so expand
        // these to the actual flags that would be used if compiled locally.
        list<string> arch_args;
        if (compiler_get_arch_flags(job,
                                    seen_march_native,
                                    seen_mcpu_native,
                                    seen_mtune_native,
                                    arch_args)) {
            ...
            for (auto it = args.begin(); it != args.end();) {
                if (it->first == "-march=native" ||
                    it->first == "-mcpu=native" ||
                    it->first == "-mtune=native") {
                    // Remove all the arguments, replace the first one with the
                    // specific flags.
                    it = args.erase(it);
                    if (!handled) {
