...
inline bool str_equal(const char *a, const char *b) { return strcmp(a, b) == 0; }
...
bool analyse_argv(const char *const *argv, CompileJob &job, bool icerun, list<string> *extrafiles) {
        ...
        if (icerun) {
            ...
            if (!strcmp(a, "-E")) {
                ...
                */
                 ...
                 * Need to handle it locally then. Search if the first part after -Wa, does not
                 * start with -
                 */
                pos = a + 3;
                ...
                seen_s = true;
            } else if (!strcmp(a, "-fprofile-arcs") || !strcmp(a, "-ftest-coverage") ||
                       !strcmp(a, "-frepo") || !strcmp(a, "-fprofile-generate") ||
                       !strcmp(a, "-fprofile-use") || !strcmp(a, "-save-temps") ||
                       !strcmp(a, "--save-temps") || !strcmp(a, "-fbranch-probabilities")) {
                log_info() << "compiler will emit profile info (argument " << a
                           << "); building locally" << endl;
                always_local = true;
                args.append(a, Arg_Local);
                ...
                if (const char *opt = argv[i + 1]) {
                    ...
                    if (str_equal(opt, "c++") || str_equal(opt, "c")) {
                        ...
                        job.setLanguage(
                            lang); // will cause -x used remotely twice, but shouldn't be a problem
                        unsupported = false;
                    }
                }
                if (unsupported) {
            ...
            } else if (!strcmp(a, "-fexec-charset") || !strcmp(a, "-fwide-exec-charset") ||
                       !strcmp(a, "-finput-charset")) {
#if CLIENT_DEBUG
                log_info() << "-f*-charset assumes charset conversion in the build environment; "
                              "must be local"
                           << endl;
#endif
                always_local = true;
                ...
                if (argv[i + 1]) {
                    ...
                    if (dot_index != string::npos) {
                        ...
                        if (ext[0] != 'h' && ext[0] != 'H' &&
                            (access(p.c_str(), R_OK) || access((p + ".gch").c_str(), R_OK))) {
                        ...
                        always_local =
                            true; /* Included file is not header.suffix or header.suffix.gch! */
                    }
                ...
                if (argv[i + 1]) {
                    ...
                    args.append(argv[i], Arg_Cpp);
                       ...
                       str_equal("--include-directory", a) || str_equal("-L", a) ||
                       str_equal("-l", a) || str_equal("--library-directory", a) ||
                       str_equal("-MF", a) || str_equal("-MT", a) || str_equal("-MQ", a) ||
                       str_equal("-cxx-isystem", a) || str_equal("-c-isystem", a) ||
                       str_equal("-idirafter", a) || str_equal("--include-directory-after", a) ||
                       str_equal("-iframework", a) || str_equal("-iframeworkwithsysroot", a) ||
                       str_equal("-imacros", a) || str_equal("-imultilib", a) ||
                       str_equal("-iprefix", a) || str_equal("--include-prefix", a) ||
                       str_equal("-iquote", a) || str_equal("-isysroot", a) ||
                       str_equal("-isystem", a) || str_equal("-isystem-after", a) ||
                       str_equal("-ivfsoverlay", a) || str_equal("-iwithprefix", a) ||
                ...
                if (argv[i + 1]) {
                    ...
                    if (str_startswith("-O", argv[i])) {
                        always_local = true;
                        log_info()
                            << "argument " << a << " " << argv[i] << ", building locally" << endl;
                    }
                ...
                args.append(a, Arg_Cpp);
            } else if (str_equal("-nostdinc", a) || str_equal("-nostdinc++", a) ||
                       str_equal("-MD", a) || str_equal("-MMD", a) || str_equal("-MG", a) ||
                       str_equal("-MP", a)) {
                args.append(a, Arg_Local);
                    ...
                    if (str_equal("-load", p)) {
                        if (argv[i + 1] && argv[i + 2] && str_equal(argv[i + 1], "-Xclang")) {
                            ...
                            if (access(file.c_str(), R_OK) == 0) {
                                ...
                                always_local = true;
                                log_info()
                                    << "plugin for argument " << a << " " << p << " " << argv[i + 1]
                                    << " " << file << " missing, building locally" << endl;
                            }
    ...
    if (!always_local) {
        ...
        if (ifile.find('.') != string::npos) {
            ...
            if (ext == "cc" || ext == "cpp" || ext == "cxx" || ext == "cp" || ext == "c++" ||
                ext == "C" || ext == "ii") {
#if CLIENT_DEBUG
    ...
    // if this could possibly even work, given that macros may be used as filenames for #include
    // directives.
    if (wunused_macros) {
    ...
    trace() << "scanned result: local args=" << concat_args(job.localFlags())
            ...
            << ", rest=" << concat_args(job.restFlags()) << ", local=" << always_local
            << ", compiler=" << job.compilerName() << ", lang=" << job.language() << endl;
#endif
