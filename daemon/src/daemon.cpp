...
namespace
{
constexpr int min_mem_limit = 100;
}

Daemon::Daemon(int& mem_limit, unsigned int& max_kids, volatile const sig_atomic_t& exit_main_loop) : mem_limit{mem_limit}, max_kids{max_kids}, exit_main_loop{exit_main_loop}
{
...
bool Daemon::setup_listen_tcp_fd( int& fd, const std::string& interface )
{
    if( !interface.empty())
        trace() << "starting to listen on interface " << interface << std::endl;
    else
        trace() << "starting to listen on all interfaces" << std::endl;
    ...
    if (getenv("ICECC_TEST_SOCKET") == nullptr) {
        ...
        if (capng_have_capability( CAPNG_EFFECTIVE, CAP_SYS_CHROOT )) {
            ...
            if(default_socket.length() > sizeof(myaddr.sun_path) - 1) {
                log_error() << "default socket path too long for sun_path" << std::endl;
            }
            if (-1 == unlink(myaddr.sun_path) && errno != ENOENT){
                log_perror("unlink failed") << "\t" << myaddr.sun_path << std::endl;
            }
            old_umask = umask(0);
            reset_umask = true;
            ...
            if( getenv( "HOME" )) {
                ...
                if(socket_path.length() > sizeof(myaddr.sun_path) - 1) {
                    log_error() << "$HOME/.iceccd.socket path too long for sun_path" << std::endl;
                }
                if (-1 == unlink(myaddr.sun_path) && errno != ENOENT){
                    log_perror("unlink failed") << "\t" << myaddr.sun_path << std::endl;
                }
        ...
        if(test_socket.length() > sizeof(myaddr.sun_path) - 1) {
            log_error() << "$ICECC_TEST_SOCKET path too long for sun_path" << std::endl;
        }
        if (-1 == unlink(myaddr.sun_path) && errno != ENOENT){
            log_perror("unlink failed") << "\t" << myaddr.sun_path << std::endl;
        }
...
void Daemon::determine_supported_features()
{
    ...
    if( archive_read_support_filter_gzip(a) < ARCHIVE_WARN ) // error
        log_error() << "No support for uncompressing gzip available." << std::endl;
...
{
    log_info() << "reannounce_environments " << std::endl;
...
int Daemon::scheduler_no_cs(NoCSMsg *msg)
{
    ...
    c->usecsmsg = new UseCSMsg(std::string(), "127.0.0.1", daemon_port, msg->job_id, true, 1, 0);
    c->status = Client::PENDING_USE_CS;
...
{
    log_info() << "handle_transfer_env, client status " << Client::status_str(client->status) <<  std::endl;
    ...
    trace() << "PID of child thread running untaring environment: " << pid << std::endl;
...
bool Daemon::handle_file_chunk_env(Client *client, Msg *msg)
{
    ...
    if (msg->type == M_END) {
        trace() << "received end of environment, waiting for child" << std::endl;
    ...
    log_error() << "protocol error while receiving environment (" << msg->type << ")" << std::endl;
...
bool Daemon::handle_env_install_child_done(Client *client)
{
    ...
    log_info() << "handle_env_install_child_done PID " << client->child_pid << " for " << client->outfile
               << " status: " << ( success ? "success" : "failed" ) << std::endl;
    client->child_pid = -1;
...
bool Daemon::finish_transfer_env(Client *client, bool cancel)
{
    ...
    if (client->child_pid >= 0 ) {
        ...
        int status;
        trace() << "finish_transfer_env kill and waiting for child PID " << client->child_pid << std::endl;
    ...
    if( !cancel ) {
        installed_size = finalize_install_environment(envbasedir, client->outfile,
                                                      user_uid, user_gid);
        log_info() << "installed_size: " << installed_size << std::endl;
    }
    if( installed_size == 0 )
...
void Daemon::check_cache_size(const std::string &new_env)
{
    ...
    while (cache_size > cache_size_limit) {
        ...
        for (const auto& it : received_environments ) {
            trace() << "considering cached environment: " << it.first << " " << it.second.last_use << " " << oldest_time << std::endl;

            if (access(std::string(envbasedir + "/target=" + it.first + "/usr/bin/as").c_str(), X_OK) != 0) {
                trace() << std::string(envbasedir + "/target=" + it.first + "/usr/bin/as") << " is missing, removing environment" << std::endl;
        ...
        for (const auto& it : native_environments ) {
            trace() << "considering native environment: " << it.first << " " << it.second.last_use << " " << oldest_time << std::endl;

            if (!it.second.name.empty() && access(it.second.name.c_str(), R_OK) != 0) {
                trace() << it.second.name << " is missing, removing environment" << std::endl;
                // force removing this one
                oldest_time = 0;
...
void Daemon::remove_environment(const std::string& env_key)
{
    assert(!env_key.empty());
    remove_environment_files(envbasedir, env_key);
    const ReceivedEnvironment& env = received_environments[env_key];
    trace() << "removing " << envbasedir << "/target=" << env_key << " " << env.size << std::endl;
    assert( cache_size >= env.size );
    cache_size -= env.size;
...
bool Daemon::handle_get_native_env(Client *client, GetNativeEnvMsg *msg)
{
    ...
    if( !IS_PROTOCOL_41(client->channel) && compiler.find('/') == std::string::npos)
    ...
    trace() << "get_native_env for " << msg->compiler
            << " (" << ccompiler << "," << cppcompiler << ")" << std::endl;

    if (stat(ccompiler.c_str(), &st) != 0) {
        log_error() << "Compiler binary " << ccompiler << " for environment not found." << std::endl;
...
void Daemon::handle_old_request()
{
    while ((current_kids + clients.active_processes) < std::max((unsigned int)1, max_kids)) {
        ...
        if (client) {
            ...
            trace() << "request for job " << job->jobID() << std::endl;
...
bool Daemon::handle_compile_done(Client *client)
{
    ...
    if(end_status == EXIT_COMPILER_MISSING) { // Environment damaged?
        ...
        if( !reannounce_environments())
            log_warning() << "failed reannounce environments after failed compile " << client->job->jobID() << std::endl;
    }

    if(!send_scheduler(*msg))
        log_warning() << "failed sending scheduler about compile done " << client->job->jobID() << std::endl;
...
{
    trace() << "handle_end " << client->client_id << " " << client->channel->name << std::endl;
#ifdef ICECC_DEBUG
...
void Daemon::answer_client_requests()
{
    ...
    for (std::map<int, MsgChannel *>::const_iterator it = fd2chan.begin();
         it != fd2chan.end();) {
        int i = it->first;
    ...
    if (ret > 0) {
        ...
        if (listen_fd != -1) {
            ...
            trace() << "accepted " << c->fd << " " << c->name << " as " << client->client_id << std::endl;
...
bool Daemon::reconnect()
{
    ...
    if (!discover && next_scheduler_connect > time(nullptr)) {
        trace() << "Delaying reconnect." << std::endl;
        return false;
    }
    ...
    if (!scheduler) {
        log_warning() << "scheduler not yet found/selected." << std::endl;
        return false;
    }
